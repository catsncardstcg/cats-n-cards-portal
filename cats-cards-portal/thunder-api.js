 ...
 * @return {Promise<Object>} Thunder API response
 */
 ...
 * @return {Promise<boolean>} True if connection works
 */
async function testThunderConnection() {
  ...
  try {
    const form = new FormData();
    // Send empty form to test connection
    // (will fail but tells us if API is reachable)
    await axios.post(
        THUNDER_API_URL,
        form,
        {
    ...
    if (error.response &&
      ...
      console.log("[Thunder API] ✅ Connection test successful " +
          "(API is reachable)");
      return true;
    }
    throw error;
